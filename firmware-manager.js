...
export class FirmwareManager{
  ...
  refreshPort(){const port=this.app.port||this.app.authorizedPorts[0],available=Boolean(port),controllable=this.app.canInstallFirmware?.()??Boolean(this.app.portIsOpen);let label='No seleccionado';if(port){try{const info=port.getInfo?.()||{};const vid=info.usbVendorId?.toString(16).toUpperCase().padStart(4,'0'),pid=info.usbProductId?.toString(16).toUpperCase().padStart(4,'0');label=vid&&pid?`USB ${vid}:${pid}`:'Puerto Serial autorizado';if(info.usbVendorId===0x2341&&info.usbProductId===0x0043){label=`Arduino Uno · USB ${vid}:${pid}`;this.e.board.value='uno';}const megaPids=[0x0010,0x0042,0x0210,0x0242];if(info.usbVendorId===0x2341&&megaPids.includes(info.usbProductId))this.e.board.value='mega';}catch{label='Puerto Serial autorizado';}}this.e.port.textContent=label;this.e.install.disabled=!available||this.busy||!controllable;if(available&&!controllable&&!this.busy&&this.app.externalPortBusy)this.setState('Puerto ocupado','No se puede instalar firmware porque el puerto está ocupado.',0);}
  ...
  async install(){
    ...
    if(!this.app.canInstallFirmware?.())return this.setState('Puerto ocupado','No se puede instalar firmware porque el puerto está ocupado. Conecte primero el Arduino y obtenga control real del puerto.',0);
    ...
    try{
      this.setState('Preparando carga...','Liberando profundamente el modo Workbench.',2);await this.app.cleanupSerialConnection('firmware-upload',{keepUI:true,keepOwnership:true,recovery:true});
      ...
      this.setState('Reiniciando Arduino...','Cambiando a modo Workbench.',100);await sleep(1200);this.app.port=port;await this.app.openPort({skipOwnership:true});
