...
const requiredIds=['connectBtn','searchDeviceBtn','reconnectBtn','testPingBtn','recoverSerialBtn','takeControlBtn','resetSerialBtn','retryDiagnosticsBtn','commandForm','clearBtn','pauseBtn','resetMetrics','stressBtn','installFirmwareBtn','firmwareBoard','checkUiControls','checkFirmwareResources','environmentInfo'];
for(const id of requiredIds)assert.match(html,new RegExp(`id=["']${id}["']`),`Falta #${id}`);
assert.match(html,/window\.__HW_BUILD_VERSION__\s*=\s*'2026\.08\.27\.1'/);
...
globalThis.window={__HW_BUILD_VERSION__:'2026.08.27.1',isSecureContext:true,self:{},top:{},addEventListener(){}};window.top=window.self;
...
assert.equal(element('recoverSerialBtn').hidden,false);
assert.match(source,/recoverSerialPort/);
assert.match(source,/BroadcastChannel/);
assert.match(source,/pagehide/);
assert.match(html,/id="recoverSerialBtn"[^>]*hidden/);
